...
| :--- | :--- | :--- | :--- |
| **Q** - Question | GitHub Copilot (Claude 4.6), Google Gemini (3.1 Pro) | Recherche Datensätze, Projektkonzept (`C-Phase.ipynb`), Erstellung der Leitdokumente (`AGENT.md`, `README.md`) | "Finde aktuelle öffentliche Datensätze zu historischen lokalisierbaren Daten zur Erderwärmung... / Schreibe hieraus eine AGENT.md für mein Projekt." |
| **U** - Understanding | Google Gemini (3.1 Pro), GitHub Copilot (Claude 4.6) | Explorative Datenanalyse (EDA) in der `U-Phase.ipynb`, Unterstützung bei spezifischen Geodaten-Bibliotheken (`xarray`, `rasterio`) | "Wie lade und visualisiere ich NetCDF-Klimadaten mit xarray und untersuche die Dimensionen?" |
| **A** - Algorithm Selection | GitHub Copilot (Claude 4.6) | Auswahl und Implementierung von Regressionsmodellen (`Ridge`, `RandomForest`) in `A-Phase.ipynb`. Vergleich von linearen und nicht-linearen Ansätzen. | "Welche Scikit-Learn Modelle eignen sich für Zeitreihenvorhersage mit wenigen Datenpunkten? Erstelle einen Vergleich." |
| **A** - data Adaption | GitHub Copilot (Claude 4.6) | Feature Engineering: Erstellung von Lag-Features und Rolling-Averages zur Abbildung von klimatischer Trägheit. | "Erzeuge Lag-Features für CO2 (1, 3, 5 Jahre) in Pandas, um verzögerte Temperatureffekte zu modellieren." |
| **A** - parameter Adjustment | GitHub Copilot (Claude 4.6) | Hyperparameter-Tuning mittels `GridSearchCV` und `TimeSeriesSplit` zur Vermeidung von Data Leakage. | "Wie verhindere ich Data Leakage bei der Cross-Validation von Zeitreihen? Implementiere TimeSeriesSplit für Ridge." |
| **C** - Conclusion & Comparison | GitHub Copilot (Claude 4.6, Gemini 3.1 Pro) | Bewertung der Modellgüte und Selektion des finalen Modells, Einbau von Konfidenzintervallen zur Vermeidung von Scheingenauigkeit. | "Vergleiche die RMSE-Werte der Modelle und visualisiere die Residuen sowie erstelle eine Metrik für Schwankungsbreiten." |
| **K** - Knowledge Transfer | GitHub Copilot (Gemini 3.1 Pro Preview, Claude 4.6), UI Refactoring Tools | Erstellung von verständlicher Dokumentation (`README.md`, `GLOSSARY.md`, `K-Phase.md`). Kompletter Aufbau, Restrukturierung und Refactoring der Streamlit-App. Konvertierung von Open-Meteo-REST zu AWS S3 (COGs via boto3/rasterio). | "Schreibe den Flood Mapper komplett auf Copernicus DEM via AWS um. Ersetze die Säulen in PyDeck durch PolygonLayers. Räume die Projektstruktur auf." | 
